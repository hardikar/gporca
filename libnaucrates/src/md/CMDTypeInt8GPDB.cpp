	...
	m_mp(mp)
{
	...
	m_mdid->AddRef();
	m_datum_null = GPOS_NEW(mp) CDatumInt8GPDB(m_mdid, 1 /* m_bytearray_value */, true /* is_null */);
}
	...
	const
{
	...
	return GPOS_NEW(m_mp) CDatumInt8GPDB(m_mdid->Sysid(), datum_dxl->Value(), datum_dxl->IsNull());
}
	...
	const
{
	...
	m_mdid->AddRef();
	CDXLDatumInt8 *datum_dxl = GPOS_NEW(mp) CDXLDatumInt8(mp, m_mdid, int8gpdb_datum->IsNull(), int8gpdb_datum->Value());

	...
	const
{
	...
	return GPOS_NEW(m_mp) CDatumBoolGPDB(m_mdid->Sysid(), datum_dxl->GetValue(), datum_dxl->IsNull());
}
	...
	const
{
	...
	m_mdid->AddRef();
	CDXLDatumBool *datum_dxl = GPOS_NEW(mp) CDXLDatumBool(mp, m_mdid, datum_bool_gpdb->IsNull(), datum_bool_gpdb->GetValue());

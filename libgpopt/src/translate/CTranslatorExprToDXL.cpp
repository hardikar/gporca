	...
	)
{
	...
	if (is_index_nlj && GPOS_FTRACE(EopttraceIndexedNLJOuterRefAsParams))
	{
		...
		col_refs = GPOS_NEW(m_memory_pool) DrgPdxlcr(m_memory_pool);
		for (ULONG ul = 0; ul < outer_refs->Size(); ul++)
		{
			CColRef *col_ref = (*outer_refs)[ul];
			CMDName *md_name = GPOS_NEW(m_memory_pool) CMDName(m_memory_pool, col_ref->Name().Pstr());
			IMDId *mdid = col_ref->Pmdtype()->MDId();
			mdid->AddRef();
			CDXLColRef *colref_dxl = GPOS_NEW(m_memory_pool) CDXLColRef(m_memory_pool, md_name, col_ref->Id(), mdid, col_ref->TypeModifier());
			col_refs->Append(colref_dxl);
		}
	}

	// construct a join node
	CDXLPhysicalNLJoin *pdxlopNLJ = GPOS_NEW(m_memory_pool) CDXLPhysicalNLJoin(m_memory_pool, join_type, is_index_nlj, nest_params_exists);

	...
	)
{
	...
	*pfEQComparison = *pfEQComparison || IMDType::EcmptEq == ecmpt;
	...
	// WIP: If the pexprPartKey is not comparable with pexprOther, then we need to add a cast on top of pexprPartKey
	// Then, use the casted PartKey operator to generate the pmdidTypeCastExpr and pmdidCastFunc
	if(CMDAccessorUtils::FCastExists(m_pmda, pmdidTypePartKey, pmdidTypeOther))
	{

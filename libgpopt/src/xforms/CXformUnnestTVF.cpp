	...
	)
{
	...
	CColRefArray *pdrgpcrProdOutput = pexprCTEProd->PcrsOutput()->Pdrgpcr(mp);
	CColRefArray *pdrgpcrConsOutput = pexprCTECons->PcrsOutput()->Pdrgpcr(mp);
	...
	for (ULONG ulOuter = 0; ulOuter < ulPrjElems; ulOuter++)
	{
		...
		if ((*pexprPrjElem)[0]->DerivePropsScalar()->FHasSubquery())
		{

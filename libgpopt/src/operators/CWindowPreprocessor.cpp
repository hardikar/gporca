	...
	)
{
	...
	CColRefSet *pcrsChildOutput = pexprChild->PcrsOutput();
	...
	CColRefArray *pdrgpcrProducerOutput = pexprCTEProd->PcrsOutput()->Pdrgpcr(mp);
	...
	)
{
	GPOS_ASSERT(NULL != pexprSeqPrj);
	GPOS_ASSERT(COperator::EopLogicalSequenceProject == pexprSeqPrj->Pop()->Eopid());
	GPOS_ASSERT(0 < (*pexprSeqPrj)[1]->DerivePropsScalar()->UlDistinctAggs());
	...
	CExpression *pexprChild = (*pexprSeqPrj)[0];
	CColRefArray *pdrgpcrChildOutput = pexprChild->PcrsOutput()->Pdrgpcr(mp);
	...
	)
{
	...
	if (COperator::EopLogicalSequenceProject == pop->Eopid() &&
		0 < (*pexpr)[1]->DerivePropsScalar()->UlDistinctAggs())
	{

	...
	)
{
	...
	CColRefSet *pcrsUsed =
		pexprPred->DerivePropsScalar()->PcrsUsed();
	CColRefSet *pcrsOutput =
		pexprLogical->PcrsOutput();
	...
	)
{
	...
	CColRefSet *pcrsOutput = pexprOuterJoinInnerChild->PcrsOutput();
	...
	)
{
	...
	CColRefSet *pcrsOutputLeftChild = pexprLeftChild->PcrsOutput();
	...
	)
{
	...
	if (0 < pdrgpexprPrElPullUp->Size() && COperator::EopLogicalProject == pop->Eopid())
	{
		...
		*pfSuccess = true;
		...
		availableCRs->Include(pexprRelational->PcrsOutput());
		availableCRs->Include(pexprRelational->PcrsOuter());
		// check that the new project node has all the values it needs
		GPOS_ASSERT(availableCRs->ContainsAll(pexprPrjList->DerivePropsScalar()->PcrsUsed()));
	...
	*pfSuccess = true;
	CExpression *pexprPrjList = GPOS_NEW(mp) CExpression(mp, GPOS_NEW(mp) CScalarProjectList(mp), pdrgpexprPrElPullUp);
	GPOS_ASSERT(pexprOutput->PcrsOutput()->ContainsAll(pexprPrjList->DerivePropsScalar()->PcrsUsed()));
	...
	)
{
	...
	for (ULONG ul = 0; ul < ulProjElements; ul++)
	{
		...
		CColRefSet *pcrsUsedByProjElem = pexprPrEl->DerivePropsScalar()->PcrsUsed();
		...
		CDrvdPropScalar *pdpscalar = pexprPrEl->DerivePropsScalar();
	...
	)
{
	...
	if (pexpr->Pop()->FLogical())
	{
		...
		for (ULONG ul = 0; ul < arity; ul++)
		{
			...
			{
				pcrsInput->Include(exprhdl.PcrsOutput(ul));
			}
		}
